...
public final class VeinPlacementTests {
    ...
    public static void veinTagIsPopulated(final GameTestHelper helper) {
        final var placedFeatures = helper.getLevel().registryAccess().lookupOrThrow(Registries.PLACED_FEATURE);
        final var veins = placedFeatures.get(BedrockOrePlacements.OVERWORLD_VEINS).orElseThrow();
    ...
    public static void overworldBiomesGetVeins(final GameTestHelper helper) {
        ...
        final var placedFeatures = registries.lookupOrThrow(Registries.PLACED_FEATURE);
        final var veins = placedFeatures.get(BedrockOrePlacements.OVERWORLD_VEINS).orElseThrow();

        final var plains = registries.lookupOrThrow(Registries.BIOME).getOrThrow(Biomes.PLAINS);
        final var ores = plains.value().getGenerationSettings().features()
        ...
        for (final var vein : veins) {
            ...
            assertTrue(helper, "expected " + key.identifier() + " to be added to plains", ores.contains(key));
        }
    ...
    public static void veinsResolveIntoBedrockBand(final GameTestHelper helper) {
        ...
        final var bottom = level.getMinY();

        final var placedFeatures = level.registryAccess().lookupOrThrow(Registries.PLACED_FEATURE);
        for (final var vein : placedFeatures.get(BedrockOrePlacements.OVERWORLD_VEINS).orElseThrow()) {
            final var key = vein.unwrapKey().orElseThrow();
            final var heights = vein.value().placement().stream()
                .filter(AboveWorldBottomPlacement.class::isInstance)
                .toList();

            assertEquals(helper, "expected " + key.identifier() + " to place relative to the world bottom", 1, heights.size());

            for (int i = 0; i < SAMPLE_COUNT; i++) {
                final var y = heights.getFirst().getPositions(context, random, origin).findFirst().orElseThrow().getY();
                assertTrue(helper, key.identifier() + " resolved to y " + y + ", outside the bedrock band", y >= bottom && y <= bottom + VEIN_MAX_OFFSET);
            }
        }
